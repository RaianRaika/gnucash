                               ...
                               gint day[6][7],gint day_month[6][7])
{
    ...
    clc_ndays_in_month = gregorian_month_length[leap(clc_year)][clc_month];
    ...
    if (clc_month > 1)
        clc_ndays_in_prev_month = gregorian_month_length[leap(clc_year)][clc_month - 1];
    else
        clc_ndays_in_prev_month = gregorian_month_length[leap(clc_year)][12];
...
static gchar *  paint_header_helper(const gint g_yy , const gint g_mm,const gint g_dd,gint  * display_month){
   ...
   // display_month=g_malloc(sizeof(gint));
    *display_month=g_mm;
...
CalendarLib *gregorian_calendar_inti(){
    ...
    for( int i =0;i<2;i++)
    {
        for ( int j=0;j<13;j++){
            ...
            calendar_obj->month_length[i][j]=gregorian_month_length[i][j];
            calendar_obj->days_in_months[i][j]=gregorian_days_in_months[i][j];
        }

    }
    calendar_obj->days_in_months[0][13]=gregorian_days_in_months[0][13];
    calendar_obj->days_in_months[1][13]=gregorian_days_in_months[1][13];
