...
static glong year_to_days(guint year) {
    ...
    {
        sumDays+=jalali_days_in_months[leap(index)][13];
    }
...
static gboolean leap(guint year) {
    ...
    if (year > 0) {
         realYear0 = (year + 38) % 2820;
         realYear1 = (year + 39) % 2820;
    } else if (year < 0) {
         realYear0 = (year + 39) % 2820;
         realYear1 = (year + 40) % 2820;
        if (realYear0 < 0) realYear0 += 2820;
        if (realYear1 < 0) realYear1 += 2820;
    } else
        return FALSE;
     leapDays0 = realYear0*0.24219 + 0.025;  // 0.24219 ~ extra days of one year
     leapDays1 = realYear1*0.24219 + 0.025;
            frac0 = floor((leapDays0 - floor(leapDays0))*1000),
            frac1 = floor((leapDays1 - floor(leapDays1))*1000);
                               ...
                               gint day[6][7],gint day_month[6][7])
{
    ...
    clc_ndays_in_month = jalali_month_length[leap(clc_year)][clc_month];

    clc_first_day = day_of_week(clc_year, clc_month, 1);
    clc_first_day = (clc_first_day + 7 - week_start) % 7;

    /* Compute days of previous month */
    clc_month--;
    if (clc_month > 1)
        clc_ndays_in_prev_month = jalali_month_length[leap(clc_year)][clc_month];
    else
        clc_ndays_in_prev_month = jalali_month_length[leap(clc_year-1)][12];
...
CalendarLib *jalali_calendar_inti(){
    ...
    for( int i =0;i<2;i++)
    {
        for ( int j=0;j<13;j++){
            ...
            calendar_obj->month_length[i][j]=jalali_month_length[i][j];
            calendar_obj->days_in_months[i][j]=jalali_days_in_months[i][j];
        }

    }
    calendar_obj->days_in_months[0][13]=jalali_days_in_months[0][13];
    calendar_obj->days_in_months[1][13]=jalali_days_in_months[1][13];
