...
gnc_calendar_init(GncCalendar *calendar) {
  ...
  guint week_origin;
    ...
    calendar->calendarType=GREGORIAN_CALENDAR; // init default to gregorian calendar
    calendar->calendarTypePrev=GREGORIAN_CALENDAR;
...
calendar_set_month_next(GncCalendar *calendar) {
    ...
    str=calendar->calendarFunction->paint_header_helper(calendar->year,calendar->month+1,calendar->selected_day,&display_month_number);
    ...
    month_len = calendar->calendarFunction->month_length[calendar->calendarFunction->leap(year)][display_month_number+ 1];
...
calendar_set_year_prev(GncCalendar *calendar) {
    ...
    month_len = calendar->calendarFunction->month_length[calendar->calendarFunction->leap(calendar->year)][calendar->month + 1];
...
calendar_set_month_prev(GncCalendar *calendar) {
    ...
    calendar->calendarFunction->invalidate_display_day_to_gregorian(year,display_month_number+1,27,&g_y,&g_m,&g_d);
    g_m--;// should be back to the calculation month
    calendar->year=g_y;
    calendar->month=g_m;
    calendar->selected_day=g_d;
    ...
    if (month_len < calendar->selected_day) {
        ...
        if (calendar->selected_day < 0)
            calendar->selected_day =
                    calendar->selected_day + 1 + calendar->calendarFunction->month_length[calendar->calendarFunction->leap(calendar->year)][calendar->month + 1];
        gnc_calendar_select_day(calendar, calendar->selected_day);
    }
...
#endif
        // a selected day may be diffrent in diffrent calendar so I use  a over rided one for this
        display_selected_day=calendar->calendarFunction->display_selected_day(calendar->year,calendar->month+1,calendar->selected_day);
...
static void calendar_pre_init_select_day(GncCalendar *calendar,
    ...
    gint real_month;
    gint real_year;
    displayed_year=calendar->calendarFunction->paint_header_helper(calendar->year,calendar->month+1,selected_day,&displayed_month);
    real_val_displayed_year=g_ascii_strtoll(displayed_year,NULL,10);
    // here I think ! May ! should be change back to gregorian!
    calendar->calendarFunction->invalidate_display_day_to_gregorian(real_val_displayed_year,displayed_month,day
...
gnc_update_calendar_lib(GncCalendar * calendar)
{
    ...
    if( calendar  && (calendar->calendarType != calendar->calendarTypePrev) &&
            calendar->calendarType != UNDEFINED_CALENDAR)
    {
        if( calendar->calendarFunction)
        {
            switch (calendar->calendarTypePrev)
            {
                case GREGORIAN_CALENDAR :
                    gregorian_calendar_destroy(calendar->calendarFunction);
                    break;
                case JALALIAN_CALENDAR :
                    jalali_calendar_destroy(calendar->calendarFunction);
                    break;
                case UNDEFINED_CALENDAR:
                    break;
            }
...
gnc_calendar_set_calendar_type(GncCalendar *calendar,const GncCalendarType calendar_type)
{
    ...
    if(calendar && (calendar_type == GREGORIAN_CALENDAR || calendar_type == JALALIAN_CALENDAR) )
    {
        calendar->calendarTypePrev=calendar->calendarType;
    ...
    // re initilize month name and day name
